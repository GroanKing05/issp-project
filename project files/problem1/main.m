...
%% Compute autocorrelation matrices and determine step size bounds
p = 4;

% % Calculate autocorrelation for Noise1 (v2)
% R_v2 = zeros(p+1, p+1);
% for i = 1:length(v2)-(p)
%     x = v2(i:i+p);
%     R_v2 = R_v2 + x*x';
% end
% R_v2 = R_v2 / (length(v2)-(p));  % Time average autocorrelation

% % Calculate autocorrelation for Noise2 (w2)
% R_w2 = zeros(p+1, p+1);
% for i = 1:length(w2)-(p)
%     x = w2(i:i+p);
%     R_w2 = R_w2 + x*x';
% end
% R_w2 = R_w2 / (length(w2)-(p));  % Time average autocorrelation
...
% lambda_max_v2 = max(eig_v2);
% lambda_max_w2 = max(eig_w2);

% % Determine step size bounds
% mu_bound_v2 = 2/lambda_max_v2;
% mu_bound_w2 = 2/lambda_max_w2;

% fprintf('Step Size Bounds:\n');
% fprintf('Signal 1 (Noise v2): 0 < mu < %.6f\n', mu_bound_v2);
% fprintf('Signal 2 (Noise w2): 0 < mu < %.6f\n', mu_bound_w2);
